---
title: "Visualizing percentages of spoken words across scenes and plays for selected characters"
...
```{r}
group1 <- c("henrik", "jeronimus", "arv", "leander")
color1 <- "#00ff33"
group2 <- c("pernille", "magdelone", "leonora", "leonore")
color2 <- "#f402fc"
group3 <- c()
color3 <- NA
group4 <- c()
...
```
```{r}
ggsave(here("graphs/word-percentage-plots2", "vis-word-percent-plot-GENDER.pdf"),
       width = 297,
       height = 210,
       units = "mm")
```
...
Plot to visualise the social status 
```{r}
source(here("src", "social_visualiser_Max.R"))


percentage_plot_status()
```
...
```{r}
ggsave(here("graphs/word-percentage-plots_Max", "20251119_vis-word-percent-plot-Max.pdf"),
       width = 297,
       height = 210,
       units = "mm")
```